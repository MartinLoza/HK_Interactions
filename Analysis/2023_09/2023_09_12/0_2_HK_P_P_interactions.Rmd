...
n_ct_interaction <- readRDS(file = paste0(out_dir,"/tmp/n_ct_P_P_interactions_v2_",date,".rds"))
```
...
```{r}
saveRDS(object = interactions_new, file = paste0(out_dir,"/interactions_P_P_v2_", date,".rds"))
```
...
THIS PROBLEM WAS CORRECTED BY SKIPPING THE FILTERING OF CP TARGETTING THEIR NEAREST GENE.

There seem to be missing region_ids. The problem is my previous filtering removing the CP targetting their nearest gene. These missing ids are kind of regions ONLY targetting their nearest gene. Maybe interesting, but for now let's ignore them.

```{r}
missing_id <- which(!interactions_cp$region_id %in% interactions_new$region_id)
missing_id <- interactions_cp[missing_id,]
```
